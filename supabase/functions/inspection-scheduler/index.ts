 ...
 *
 * Runs daily (via the Vercel cron at /api/cron/inspection-scheduler, 08:00 UTC)
 * to auto-create planned inspection records for every org that has overdue or
 ...
 *
 * NOTE: assignee email notifications are intentionally DISABLED (SEND_EMAILS=false)
 * until customer notifications are turned on. Records are still created and
 * assigned by role; flip SEND_EMAILS to true (and wire emails to auth.users —
 * the profiles table has no email column) to re-enable.
 */
...
// Email notifications are off for now (see header note).
...
async function scheduleForOrg(
  ...
  // Members for assignment. NOTE: profiles has no `email` column — emails live in
  // auth.users. Assignment only needs id + role, so we select those.
  ...
  const orgMembers: Array<{ id: string; role: string | null }> = members ?? [];
  ...
  // Assignee email notifications are intentionally disabled (SEND_EMAILS=false).
  // When re-enabling, resolve emails from auth.users (profiles has no email column).
  void SEND_EMAILS;
