...
import { ShieldCheck, Plus, AlertTriangle, Activity } from "lucide-react";
...
const FREQUENCIES = ["daily", "weekly", "monthly", "quarterly", "annual", "event_triggered", "per_change", "before_use", "per_batch"];
...
export default async function RiskRegisterPage({ searchParams }: Props) {
  ...
  const entries = await listRiskRegisterEntries({ status: statusFilter, risk: riskFilter }).catch(() => []);
  const total = entries.length;
  const overdue = entries.filter((e) => e.overdue || e.status === "overdue").length;
    ...
    <AppShell>
      <div className="page-stack">
        <header className="page-header">
          <p className="section-label">Plan · Risk Register</p>
          <h1>Risk Register</h1>
          <p className="muted">
            The core register from the platform manual. Every requirement, control, frequency, qualified
            reviewer, and evidence need lives here and feeds the Compliance Calendar.
          </p>
        </header>
        ...
        <nav className="command-center-link-strip" aria-label="Status filter">
          <Link href="/plan/risk-register" className={`button-secondary compact ${!statusFilter ? "active-filter" : ""}`}>All</Link>
        ...
        <section className="panel">
          <div className="panel-heading">
            <div><p className="section-label">Register</p><h2>{total} entr{total !== 1 ? "ies" : "y"}</h2></div>
          </div>
          {entries.length === 0 ? (
            ...
            <div className="action-list">
              {entries.map((e) => (
                <article className="action-row" key={e.id}>
                  ...
                  <form action={updateRiskRegisterStatusAction} style={{ display: "flex", gap: 6, alignItems: "center" }}>
