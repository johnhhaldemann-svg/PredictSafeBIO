...
export type AutoScheduleResult = {
  created: number;
  skipped: number;
  inspections: Array<{ id: string; inspectionType: InspectionType; assignedTo: string | null; dueDate: string }>;
};
...
// ---------------------------------------------------------------------------
// Role → Inspection Category mapping (for auto-assignment)
// ---------------------------------------------------------------------------

/**
 * Maps each inspection category to the preferred DB role(s) to assign.
 * The scheduler picks the first matching user in the org; falls back to
 * the org owner if no role match is found.
 */
export const INSPECTION_CATEGORY_ROLES: Record<string, string[]> = {
...
/**
 * Given a list of org members (profiles), pick the best assignee for a
 * given inspection category. Returns the user id or null.
 */
export function pickAssignee(
  category: string,
  members: Array<{ id: string; role: string | null }>
): string | null {
  const preferred = INSPECTION_CATEGORY_ROLES[category] ?? ["owner", "admin", "safety_manager"];
...
// ---------------------------------------------------------------------------
// Auto-schedule: create planned inspection records from AI recommendations
// ---------------------------------------------------------------------------

/**
 * Core auto-scheduling function.
 *
 * For each inspection type that is overdue or due within 30 days, creates
 * a `planned` audit record if one does not already exist.
 *
 * Called:
 *  - on org onboarding (via API route)
 *  - daily by the Supabase Edge Function cron
 *
...
export async function autoScheduleInspections(
  ...
  const completed = (completedRows ?? []).map((r: { audit_type: string; completed_at: string | null; scheduled_for: string | null }) => ({
    auditType: r.audit_type as InspectionType,
    completedAt: r.completed_at,
    scheduledFor: r.scheduled_for,
  }));

  const recommendations = computeAiInspectionRecommendations(completed);
  if (recommendations.length === 0) return { created: 0, skipped: 0, inspections: [] };

  // 2. Fetch existing planned / in_progress records to avoid duplicates
  ...
  const alreadyScheduled = new Set<string>(
    (existingRows ?? []).map((r: { audit_type: string }) => r.audit_type)
  );

  // 3. Fetch org members for role-based assignment
  ...
  // 4. Create planned records for each unscheduled recommendation
  const toCreate = recommendations.filter(
    (rec) => !alreadyScheduled.has(rec.inspectionType)
  );

  const created: AutoScheduleResult["inspections"] = [];
  let skipped = recommendations.length - toCreate.length;

  for (const rec of toCreate) {
    const assignedTo = pickAssignee(rec.category, orgMembers);
    const { data, error } = await serviceClient
      .from("audits")
      .insert({
        organization_id: orgId,
        title: rec.label,
        audit_type: rec.inspectionType,
        status: "planned",
  ...
  return { created: created.length, skipped, inspections: created };
}
...
export async function listInspections(filter?: { status?: InspectionStatus | "all" }): Promise<Inspection[]> {
  ...
  let query = supabase
    .from("audits")
    .select("id,title,audit_type,status,scheduled_for,completed_at,created_by,created_at,updated_at,organization_id,assigned_to,auto_generated,next_due_date")
  ...
  // Fetch finding counts and assignee names in parallel
  ...
  const profileMap = new Map<string, string>(
    (profileResult.data ?? []).map((p: { id: string; full_name: string | null }) => [p.id, p.full_name ?? ""])
  );
  ...
  return data.map((row) => ({
    ...
    assigneeName: row.assigned_to ? (profileMap.get(row.assigned_to) ?? null) : null,
